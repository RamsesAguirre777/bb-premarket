...
from dc.utils import _filter_bars_before, _validate_premarket_bars
...
async def run_mode_9_28(tickers_to_run: list, date_str: str, output_data: dict) -> None:
    ...
    for ticker in tickers_to_run:
        try:
            ...
            if not bars_1m_pre:
                continue
            _validate_premarket_bars(ticker, bars_dict.get("1m", []), cutoff_et)
            ...
            logger.info(
                ...
                f"precio_pm={round(precio_premarket, 4)} | "
    ...
    logger.info(
        f"=== MODO premarket_9_28 completado | OK={len(tickers_ok)} | SKIP={skips_n} ==="
    )
